...
public class StaffWorkParametersService {
    ...
    private void validateRequest(WorkParametersUpdateRequest request) {
        // Positivity checks
        if (request.minHoursPerDay() != null && request.minHoursPerDay().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("minHoursPerDay", "Minimum hours per day must be positive");
        }
        if (request.maxHoursPerDay() != null && request.maxHoursPerDay().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("maxHoursPerDay", "Maximum hours per day must be positive");
        }
        if (request.maxHoursPerMonth() != null && request.maxHoursPerMonth().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("maxHoursPerMonth", "Maximum hours per month must be positive");
        }
        if (request.minHoursPerWeek() != null && request.minHoursPerWeek().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("minHoursPerWeek", "Minimum hours per week must be positive");
        }
        if (request.maxHoursPerWeek() != null && request.maxHoursPerWeek().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("maxHoursPerWeek", "Maximum hours per week must be positive");
        }
        if (request.minHoursPerMonth() != null && request.minHoursPerMonth().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("minHoursPerMonth", "Minimum hours per month must be positive");
        }

        // Upper bound validations
        if (request.maxHoursPerDay() != null && request.maxHoursPerDay().compareTo(new BigDecimal("24")) > 0) {
            throw new ValidationException("maxHoursPerDay", "Maximum hours per day cannot exceed 24");
        }
        if (request.maxHoursPerWeek() != null && request.maxHoursPerWeek().compareTo(new BigDecimal("168")) > 0) {
            throw new ValidationException("maxHoursPerWeek", "Maximum hours per week cannot exceed 168");
        }
        if (request.maxHoursPerMonth() != null && request.maxHoursPerMonth().compareTo(new BigDecimal("744")) > 0) {
            throw new ValidationException("maxHoursPerMonth", "Maximum hours per month cannot exceed 744");
        }
        ...
        if (request.maxHoursPerMonth() != null && request.maxHoursPerDay() != null) {
            if (request.maxHoursPerMonth().compareTo(request.maxHoursPerDay()) < 0) {
                throw new ValidationException("maxHoursPerMonth", "Maximum hours per month cannot be less than max hours per day");
            }
        }
        if (request.maxHoursPerWeek() != null && request.maxHoursPerDay() != null) {
            if (request.maxHoursPerWeek().compareTo(request.maxHoursPerDay()) < 0) {
                throw new ValidationException("maxHoursPerWeek", "Maximum hours per week cannot be less than max hours per day");
            }
        }
        if (request.maxHoursPerMonth() != null && request.maxHoursPerWeek() != null) {
            if (request.maxHoursPerMonth().compareTo(request.maxHoursPerWeek()) < 0) {
                throw new ValidationException("maxHoursPerMonth", "Maximum hours per month cannot be less than max hours per week");
            }
        }
        ...
        if (request.maxSitesPerDay() != null && request.maxSitesPerDay() > 10) {
            throw new ValidationException("maxSitesPerDay", "Maximum sites per day cannot exceed 10");
        }
